...
```mermaid
flowchart TD
    A[2v2 Merge] --> B{Situation?}
    B -->|Höhenvorteil| C[Boom & Zoom auf Lead]
    B -->|Gleichstand| D[Vertical Split]
    B -->|Nachteil| E[Extension + Reset]

    C --> F[Wingman deckt / opportunistisch]
    D --> G[Du hoch, Wingman tief]
    E --> H[Beide beschleunigen, neu starten]
```
...
```mermaid
flowchart LR
    A[Merge] --> B[Beide vertikal]
    B --> C[Höhenversatz: 3-5k ft]
    C --> D[Abwechselnde Angriffe]
    D --> E[Gegner kann nie Energie aufbauen]
```
...
```mermaid
flowchart TD
    A[T-18 engaged tief] --> B[Gegner fokussiert T-18]
    B --> C[T-15 Boom & Zoom von oben]
    C --> D{Treffer?}
    D -->|Ja| E[Kill]
    D -->|Nein| F[T-15 zieht hoch, T-18 kämpft weiter]
    F --> B
```
...
```mermaid
flowchart TD
    A[1v2 Situation] --> B[Zoom Climb]
    B --> C{Folgen beide?}
    C -->|Ja| D[Einer wird langsamer]
    C -->|Nein| E[Angriff auf den Einzelnen]
    D --> F[Angriff auf den Langsameren]
    E --> G[Boom & Zoom]
    F --> G
    G --> H[Sofort Extension]
```
